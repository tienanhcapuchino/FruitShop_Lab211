...
public class Manager {
    ...
    static int menu() {
        ...
        System.out.println("------WELCOME TO TIEN ANH CAPUCHINO 'S SHOP------");
    ...
    // create
    static void create(ArrayList<Fruit> lf) {
            System.out.print("Enter fruit id: ");
            String fruitId = Validation.checkInputString();
            // check id exist
            if (!Validation.checkIdExist(lf, fruitId)) {
    ...
    // allow user update fruit
    public static void updateFruit(ArrayList<Fruit> f) {
        Scanner sc = new Scanner(System.in);
        int count = 0;
        String newid;
        int n = 0;
        System.out.println("Enter the ID Fruit to Update: ");
        newid= Validation.checkInputString();
        int i;
        for (i = 0; i < f.size(); i++) {
            if (f.get(i).getFruitId().equalsIgnoreCase(newid)) {
                count++;
                break;
            }
        }
        if (count > 0) {
            ...
            f.get(i).setQuantity(n);
        }
        if (count == 0) {
            System.out.println("This fruit isnot available.");
            if (Validation.checkInputYN()) {
                    create(f);
                    displayListFruit(f);
    ...
    static void viewOrder(Hashtable<String, ArrayList<Order>> ht) {
        if (ht.isEmpty()) {
            System.err.println("Nothing is in the list!");
            System.err.println("Please go shopping!");
        } else {
    ...
    static void shopping(ArrayList<Fruit> lf, Hashtable<String, ArrayList<Order>> ht) {
        ...
        System.err.println("THANKS FOR BUYING!");
    }
